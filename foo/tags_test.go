...
func testTagsDelete(t *testing.T) {
	...
	var err error
	o := &Tag{}
	if err = randomize.Struct(seed, o, tagDBTypes, true, tagTestRandomBlacklist...); err != nil {
...
func testTagsAll(t *testing.T) {
	...
	tagOne := &Tag{}
	tagTwo := &Tag{}
	if err = randomize.Struct(seed, tagOne, tagDBTypes, false, tagTestRandomBlacklist...); err != nil {
		t.Errorf("Unable to randomize Tag struct: %s", err)
	}
	if err = randomize.Struct(seed, tagTwo, tagDBTypes, false, tagTestRandomBlacklist...); err != nil {
		t.Errorf("Unable to randomize Tag struct: %s", err)
	}
...
func testTagToManyBookmarks(t *testing.T) {
	...
	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, tagDBTypes, true, tagTestRandomBlacklist...); err != nil {
		t.Errorf("Unable to randomize Tag struct: %s", err)
	}
	...
	_, err = tx.Exec("insert into [dbo].[bookmark_contexts] ([tag_id], [bookmark_id]) values ($1, $2)", a.ID, b.ID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into [dbo].[bookmark_contexts] ([tag_id], [bookmark_id]) values ($1, $2)", a.ID, c.ID)
	if err != nil {
		t.Fatal(err)
	}
...
func testTagToManyDocuments(t *testing.T) {
	...
	_, err = tx.Exec("insert into [dbo].[document_contexts] ([tag_id], [document_id]) values ($1, $2)", a.ID, b.ID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into [dbo].[document_contexts] ([tag_id], [document_id]) values ($1, $2)", a.ID, c.ID)
	if err != nil {
		t.Fatal(err)
	}
...
func testTagToManyParentTagTags(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, tagDBTypes, false, tagTestRandomBlacklist...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, tagDBTypes, false, tagTestRandomBlacklist...); err != nil {
		t.Fatal(err)
	}

	queries.Assign(&b.ParentTag, a.ID)
	queries.Assign(&c.ParentTag, a.ID)
	if err = b.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}
	if err = c.Insert(ctx, tx, boil.Infer()); err != nil {
	...
	check, err := a.ParentTagTags().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	bFound, cFound := false, false
	for _, v := range check {
		if queries.Equal(v.ParentTag, b.ParentTag) {
			bFound = true
		}
		if queries.Equal(v.ParentTag, c.ParentTag) {
			cFound = true
		}
	}
	...
	slice := TagSlice{&a}
	if err = a.L.LoadParentTagTags(ctx, tx, false, (*[]*Tag)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ParentTagTags); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.ParentTagTags = nil
	if err = a.L.LoadParentTagTags(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ParentTagTags); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testTagToManyAddOpParentTagTags(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddParentTagTags(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if !queries.Equal(a.ID, first.ParentTag) {
			t.Error("foreign key was wrong value", a.ID, first.ParentTag)
		}
		if !queries.Equal(a.ID, second.ParentTag) {
			t.Error("foreign key was wrong value", a.ID, second.ParentTag)
		}

		if first.R.ParentTagTag != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}
		if second.R.ParentTagTag != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}

		if a.R.ParentTagTags[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.ParentTagTags[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.ParentTagTags().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
func testTagToManySetOpParentTagTags(t *testing.T) {
	var err error
	...
	err = a.SetParentTagTags(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetParentTagTags(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
	...
	if !queries.IsValuerNil(b.ParentTag) {
		t.Error("want b's foreign key value to be nil")
	}
	if !queries.IsValuerNil(c.ParentTag) {
		t.Error("want c's foreign key value to be nil")
	}
	if !queries.Equal(a.ID, d.ParentTag) {
		t.Error("foreign key was wrong value", a.ID, d.ParentTag)
	}
	if !queries.Equal(a.ID, e.ParentTag) {
		t.Error("foreign key was wrong value", a.ID, e.ParentTag)
	}

	if b.R.ParentTagTag != nil {
		t.Error("relationship was not removed properly from the foreign struct")
	}
	if c.R.ParentTagTag != nil {
		t.Error("relationship was not removed properly from the foreign struct")
	}
	if d.R.ParentTagTag != &a {
		t.Error("relationship was not added properly to the foreign struct")
	}
	if e.R.ParentTagTag != &a {
		t.Error("relationship was not added properly to the foreign struct")
	}

	if a.R.ParentTagTags[0] != &d {
		t.Error("relationship struct slice not set to correct value")
	}
	if a.R.ParentTagTags[1] != &e {
		t.Error("relationship struct slice not set to correct value")
	}
}

func testTagToManyRemoveOpParentTagTags(t *testing.T) {
	var err error
	...
	err = a.AddParentTagTags(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveParentTagTags(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(a.R.ParentTagTags) != 2 {
		t.Error("should have preserved two relationships")
	}

	// Removal doesn't do a stable deletion for performance so we have to flip the order
	if a.R.ParentTagTags[1] != &d {
		t.Error("relationship to d should have been preserved")
	}
	if a.R.ParentTagTags[0] != &e {
		t.Error("relationship to e should have been preserved")
	}
}

func testTagToOneTagUsingParentTagTag(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	seed := randomize.NewSeed()
	if err := randomize.Struct(seed, &local, tagDBTypes, true, tagTestRandomBlacklist...); err != nil {
		t.Errorf("Unable to randomize Tag struct: %s", err)
	}
	if err := randomize.Struct(seed, &foreign, tagDBTypes, false, tagTestRandomBlacklist...); err != nil {
		t.Errorf("Unable to randomize Tag struct: %s", err)
	}
	...
	queries.Assign(&local.ParentTag, foreign.ID)
	if err := local.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}

	check, err := local.ParentTagTag().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = local.L.LoadParentTagTag(ctx, tx, false, (*[]*Tag)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.ParentTagTag == nil {
		t.Error("struct should have been eager loaded")
	}

	local.R.ParentTagTag = nil
	if err = local.L.LoadParentTagTag(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.ParentTagTag == nil {
		t.Error("struct should have been eager loaded")
...
func testTagToOneSetOpTagUsingParentTagTag(t *testing.T) {
	var err error
	...
	for i, x := range []*Tag{&b, &c} {
		err = a.SetParentTagTag(ctx, tx, i != 0, x)
		if err != nil {
			t.Fatal(err)
		}

		if a.R.ParentTagTag != x {
			t.Error("relationship struct not set to correct value")
		}

		if x.R.ParentTagTags[0] != &a {
			t.Error("failed to append to foreign relationship struct")
		}
		if !queries.Equal(a.ParentTag, x.ID) {
			t.Error("foreign key was wrong value", a.ParentTag)
		}

		zero := reflect.Zero(reflect.TypeOf(a.ParentTag))
		reflect.Indirect(reflect.ValueOf(&a.ParentTag)).Set(zero)
		...
		if !queries.Equal(a.ParentTag, x.ID) {
			t.Error("foreign key was wrong value", a.ParentTag, x.ID)
		}
	}
}

func testTagToOneRemoveOpTagUsingParentTagTag(t *testing.T) {
	var err error
	...
	if err = a.SetParentTagTag(ctx, tx, true, &b); err != nil {
		t.Fatal(err)
	}

	if err = a.RemoveParentTagTag(ctx, tx, &b); err != nil {
		t.Error("failed to remove relationship")
	}

	count, err := a.ParentTagTag().Count(ctx, tx)
	if err != nil {
		t.Error(err)
	}
	if count != 0 {
	...
	if a.R.ParentTagTag != nil {
		t.Error("R struct entry should be nil")
	}

	if !queries.IsValuerNil(a.ParentTag) {
		t.Error("foreign key value should be nil")
	}

	if len(b.R.ParentTagTags) != 0 {
...
var (
	tagDBTypes = map[string]string{`ID`: `int`, `ParentTag`: `int`, `Tag`: `varchar`, `Depth`: `int`}
	_          = bytes.MinRead
)
