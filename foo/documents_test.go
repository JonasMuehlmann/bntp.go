...
func testDocumentToOneDocumentTypeUsingDocumentType(t *testing.T) {
	...
	queries.Assign(&local.DocumentTypeID, foreign.ID)
...
func testDocumentToOneSetOpDocumentTypeUsingDocumentType(t *testing.T) {
	...
	for i, x := range []*DocumentType{&b, &c} {
		...
		if !queries.Equal(a.DocumentTypeID, x.ID) {
...
func testDocumentToOneRemoveOpDocumentTypeUsingDocumentType(t *testing.T) {
	var err error
	...
	if err = a.SetDocumentType(ctx, tx, true, &b); err != nil {
		t.Fatal(err)
	}

	if err = a.RemoveDocumentType(ctx, tx, &b); err != nil {
		t.Error("failed to remove relationship")
	}

	count, err := a.DocumentType().Count(ctx, tx)
	if err != nil {
		t.Error(err)
	}
	if count != 0 {
	...
	if a.R.DocumentType != nil {
		t.Error("R struct entry should be nil")
	}

	if !queries.IsValuerNil(a.DocumentTypeID) {
