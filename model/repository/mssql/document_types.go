...
var DocumentTypeWhere = struct {
	...
	ID:           whereHelperint64{field: "[dbo].[document_types].[id]"},
...
maybedocumentTypeGeneratedColumns      = []string{"ID"}
)
...
func (o *DocumentType) SetDocuments(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Document) error {
	query := "update [dbo].[documents] set [document_type_id] = null where [document_type_id] = $1"
...
func (o *DocumentType) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		if len(insert) == 0 {
			return errors.New("repository: unable to upsert document_types, could not build insert column list")
		}
