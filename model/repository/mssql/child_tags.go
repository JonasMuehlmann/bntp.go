...
var ChildTagWhere = struct {
	...
	ID:          whereHelperint{field: "[dbo].[child_tags].[id]"},
	ParentTagID: whereHelpernull_Int{field: "[dbo].[child_tags].[parent_tag_id]"},
	ChildTagID:  whereHelpernull_Int{field: "[dbo].[child_tags].[child_tag_id]"},
}
...
func ChildTags(mods ...qm.QueryMod) childTagQuery {
	mods = append(mods, qm.From("[dbo].[child_tags]"))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"[dbo].[child_tags].*"})
	}
...
func FindChildTag(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*ChildTag, error) {
	...
	query := fmt.Sprintf(
		"select %s from [dbo].[child_tags] where [id]=$1", sel,
	)
...
func (o *ChildTag) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO [dbo].[child_tags] ([%s]) %%sVALUES (%s)%%s", strings.Join(wl, "],["), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO [dbo].[child_tags] %sDEFAULT VALUES%s"
		}
...
func (o *ChildTag) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE [dbo].[child_tags] SET %s WHERE %s",
...
func (o ChildTagSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE [dbo].[child_tags] SET %s WHERE %s",
...
func (o *ChildTag) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		for i, v := range insert {
			if strmangle.ContainsAny(childTagPrimaryKeyColumns, v) && strmangle.ContainsAny(childTagColumnsWithDefault, v) {
				insert = append(insert[:i], insert[i+1:]...)
			}
		}
		if len(insert) == 0 {
			return errors.New("models: unable to upsert child_tags, could not build insert column list")
		}
		...
		cache.query = buildUpsertQueryMSSQL(dialect, "[dbo].[child_tags]", childTagPrimaryKeyColumns, update, insert, ret)

		whitelist := make([]string, len(childTagPrimaryKeyColumns))
		copy(whitelist, childTagPrimaryKeyColumns)
		whitelist = append(whitelist, update...)
		whitelist = append(whitelist, insert...)

		cache.valueMapping, err = queries.BindMapping(childTagType, childTagMapping, whitelist)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *ChildTag) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[child_tags] WHERE [id]=$1"
...
func (o ChildTagSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[child_tags] WHERE " +
...
func (o *ChildTagSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT [dbo].[child_tags].* FROM [dbo].[child_tags] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, childTagPrimaryKeyColumns, len(*o))
...
func ChildTagExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select case when exists(select top(1) 1 from [dbo].[child_tags] where [id]=$1) then 1 else 0 end"
