...
var DocumentWhere = struct {
	...
	ID:             whereHelperint64{field: "[dbo].[documents].[id]"},
	Path:           whereHelperstring{field: "[dbo].[documents].[path]"},
	DocumentTypeID: whereHelpernull_Int64{field: "[dbo].[documents].[document_type_id]"},
...
func (documentL) LoadTags(ctx context.Context, e boil.ContextExecutor, singular bool, maybeDocument interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.Select("[dbo].[tags].[id], [dbo].[tags].[tag], [dbo].[tags].[parent_tag], [dbo].[tags].[path], [dbo].[tags].[children], [a].[document_id]"),
		qm.From("[dbo].[tags]"),
...
func (o *Document) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		if len(insert) == 0 {
			return errors.New("repository: unable to upsert documents, could not build insert column list")
		}
