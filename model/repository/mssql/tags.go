...
var TagWhere = struct {
	...
	Depth:     whereHelperint64{field: "[dbo].[tags].[depth]"},
}
...
func (o *Tag) ParentTagTags(mods ...qm.QueryMod) tagQuery {
	...
	queryMods = append(queryMods,
		qm.Where("[dbo].[tags].[parent_tag]=?", o.ID),
	)
...
func (tagL) LoadParentTagTags(ctx context.Context, e boil.ContextExecutor, singular bool, maybeTag interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`dbo.tags`),
		qm.WhereIn(`dbo.tags.parent_tag in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *Tag) SetParentTagTags(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Tag) error {
	query := "update [dbo].[tags] set [parent_tag] = null where [parent_tag] = $1"
	values := []interface{}{o.ID}
	if boil.IsDebug(ctx) {
		writer := boil.DebugWriterFrom(ctx)
		fmt.Fprintln(writer, query)
...
func (o *Tag) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		if len(insert) == 0 {
			return errors.New("repository: unable to upsert tags, could not build insert column list")
		}
