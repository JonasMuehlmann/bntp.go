...
func TestSQLDocumentRepositoryDocumentModelConverter(t *testing.T) {
	t.Parallel()
	...
	for _, document := range documents {
		...
		assert.NoError(t, err, ", assert converting domain model with id: "+strconv.FormatInt(document.ID, 10))

		convertedBack, err := repo.DocumentRepositoryToDomainModel(context.Background(), repositoryModel.(*repository.Document))
		assert.NoError(t, err, ", assert converting repository model with id: "+strconv.FormatInt(document.ID, 10))
		assert.EqualValues(t, document, convertedBack, ", assert comparing reconverted model with id: "+strconv.FormatInt(document.ID, 10))

		domainModel, err := repo.GetFirstWhere(context.Background(), &domain.DocumentFilter{ID: optional.Make(model.FilterOperation[int64]{Operator: model.FilterEqual, Operand: model.ScalarOperand[int64]{document.ID}})})
		assert.NoError(t, err)
		assert.EqualValues(t, document, domainModel, ", assert comparing reloaded model with id: "+strconv.FormatInt(document.ID, 10))
	}
}
