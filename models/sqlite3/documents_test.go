...
func testDocumentToManyTags(t *testing.T) {
	...
	_, err = tx.Exec("insert into \"document_contexts\" (\"document_id\", \"tag_id\") values (?, ?)", a.ID, b.ID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into \"document_contexts\" (\"document_id\", \"tag_id\") values (?, ?)", a.ID, c.ID)
	if err != nil {
		t.Fatal(err)
	}
...
func testDocumentToManySourceDocuments(t *testing.T) {
	...
	_, err = tx.Exec("insert into \"links\" (\"destination_id\", \"source_id\") values (?, ?)", a.ID, b.ID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into \"links\" (\"destination_id\", \"source_id\") values (?, ?)", a.ID, c.ID)
	if err != nil {
		t.Fatal(err)
	}
...
func testDocumentToManyDestinationDocuments(t *testing.T) {
	...
	_, err = tx.Exec("insert into \"links\" (\"source_id\", \"destination_id\") values (?, ?)", a.ID, b.ID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into \"links\" (\"source_id\", \"destination_id\") values (?, ?)", a.ID, c.ID)
	if err != nil {
		t.Fatal(err)
	}
...
var (
	documentDBTypes = map[string]string{`ID`: `INTEGER`, `Path`: `TEXT`, `DocumentTypeID`: `INTEGER`, `CreatedAt`: `TIMESTAMP`, `UpdatedAt`: `TIMESTAMP`, `DeletedAt`: `TIMESTAMP`}
	_               = bytes.MinRead
)
...
func testDocumentsSliceUpdateAll(t *testing.T) {
	...
	if strmangle.StringSliceMatch(documentAllColumns, documentPrimaryKeyColumns) {
		...
		fields = strmangle.SetComplement(fields, documentGeneratedColumns)
	}
