...
var BookmarkTypeWhere = struct {
	...
	ID:   whereHelperint{field: "[dbo].[bookmark_types].[id]"},
	Type: whereHelperstring{field: "[dbo].[bookmark_types].[Type]"},
}
...
func (o *BookmarkType) Bookmarks(mods ...qm.QueryMod) bookmarkQuery {
	...
	queryMods = append(queryMods,
		qm.Where("[dbo].[bookmarks].[bookmark_type_id]=?", o.ID),
	)
...
func (bookmarkTypeL) LoadBookmarks(ctx context.Context, e boil.ContextExecutor, singular bool, maybeBookmarkType interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		...
		qm.WhereIn(`dbo.bookmarks.bookmark_type_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *BookmarkType) SetBookmarks(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Bookmark) error {
	query := "update [dbo].[bookmarks] set [bookmark_type_id] = null where [bookmark_type_id] = $1"
...
func BookmarkTypes(mods ...qm.QueryMod) bookmarkTypeQuery {
	mods = append(mods, qm.From("[dbo].[bookmark_types]"))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"[dbo].[bookmark_types].*"})
	}
...
func FindBookmarkType(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*BookmarkType, error) {
	...
	query := fmt.Sprintf(
		"select %s from [dbo].[bookmark_types] where [id]=$1", sel,
	)
...
func (o *BookmarkType) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO [dbo].[bookmark_types] ([%s]) %%sVALUES (%s)%%s", strings.Join(wl, "],["), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO [dbo].[bookmark_types] %sDEFAULT VALUES%s"
		}
...
func (o *BookmarkType) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE [dbo].[bookmark_types] SET %s WHERE %s",
...
func (o BookmarkTypeSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE [dbo].[bookmark_types] SET %s WHERE %s",
...
func (o *BookmarkType) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		for i, v := range insert {
			if strmangle.ContainsAny(bookmarkTypePrimaryKeyColumns, v) && strmangle.ContainsAny(bookmarkTypeColumnsWithDefault, v) {
				insert = append(insert[:i], insert[i+1:]...)
			}
		}
		if len(insert) == 0 {
			return errors.New("models: unable to upsert bookmark_types, could not build insert column list")
		}
		...
		cache.query = buildUpsertQueryMSSQL(dialect, "[dbo].[bookmark_types]", bookmarkTypePrimaryKeyColumns, update, insert, ret)

		whitelist := make([]string, len(bookmarkTypePrimaryKeyColumns))
		copy(whitelist, bookmarkTypePrimaryKeyColumns)
		whitelist = append(whitelist, update...)
		whitelist = append(whitelist, insert...)

		cache.valueMapping, err = queries.BindMapping(bookmarkTypeType, bookmarkTypeMapping, whitelist)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert bookmark_types")
	}
...
func (o *BookmarkType) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[bookmark_types] WHERE [id]=$1"
...
func (o BookmarkTypeSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[bookmark_types] WHERE " +
...
func (o *BookmarkTypeSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT [dbo].[bookmark_types].* FROM [dbo].[bookmark_types] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, bookmarkTypePrimaryKeyColumns, len(*o))
...
func BookmarkTypeExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select case when exists(select top(1) 1 from [dbo].[bookmark_types] where [id]=$1) then 1 else 0 end"
