...
func testBookmarkToManyTags(t *testing.T) {
	...
	_, err = tx.Exec("insert into [dbo].[bookmark_contexts] ([bookmark_id], [tag_id]) values ($1, $2)", a.ID, b.ID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into [dbo].[bookmark_contexts] ([bookmark_id], [tag_id]) values ($1, $2)", a.ID, c.ID)
	if err != nil {
		t.Fatal(err)
	}
...
var (
	bookmarkDBTypes = map[string]string{`ID`: `int`, `IsRead`: `int`, `Title`: `varchar`, `URL`: `varchar`, `TimeAdded`: `datetime`, `BookmarkTypeID`: `int`, `IsCollection`: `int`}
	_               = bytes.MinRead
)
