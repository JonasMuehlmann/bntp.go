...
var DocumentTypeWhere = struct {
	...
	ID:           whereHelperint{field: "[dbo].[document_types].[id]"},
	DocumentType: whereHelperstring{field: "[dbo].[document_types].[document_type]"},
}
...
func (o *DocumentType) Documents(mods ...qm.QueryMod) documentQuery {
	...
	queryMods = append(queryMods,
		qm.Where("[dbo].[documents].[document_type_id]=?", o.ID),
	)
...
func (documentTypeL) LoadDocuments(ctx context.Context, e boil.ContextExecutor, singular bool, maybeDocumentType interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		...
		qm.WhereIn(`dbo.documents.document_type_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func DocumentTypes(mods ...qm.QueryMod) documentTypeQuery {
	mods = append(mods, qm.From("[dbo].[document_types]"))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"[dbo].[document_types].*"})
	}
...
func FindDocumentType(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*DocumentType, error) {
	...
	query := fmt.Sprintf(
		"select %s from [dbo].[document_types] where [id]=$1", sel,
	)
...
func (o *DocumentType) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO [dbo].[document_types] ([%s]) %%sVALUES (%s)%%s", strings.Join(wl, "],["), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO [dbo].[document_types] %sDEFAULT VALUES%s"
		}
...
func (o *DocumentType) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE [dbo].[document_types] SET %s WHERE %s",
...
func (o DocumentTypeSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE [dbo].[document_types] SET %s WHERE %s",
...
func (o *DocumentType) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		for i, v := range insert {
			if strmangle.ContainsAny(documentTypePrimaryKeyColumns, v) && strmangle.ContainsAny(documentTypeColumnsWithDefault, v) {
				insert = append(insert[:i], insert[i+1:]...)
			}
		}
		if len(insert) == 0 {
			return errors.New("models: unable to upsert document_types, could not build insert column list")
		}
		...
		cache.query = buildUpsertQueryMSSQL(dialect, "[dbo].[document_types]", documentTypePrimaryKeyColumns, update, insert, ret)

		whitelist := make([]string, len(documentTypePrimaryKeyColumns))
		copy(whitelist, documentTypePrimaryKeyColumns)
		whitelist = append(whitelist, update...)
		whitelist = append(whitelist, insert...)

		cache.valueMapping, err = queries.BindMapping(documentTypeType, documentTypeMapping, whitelist)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *DocumentType) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[document_types] WHERE [id]=$1"
...
func (o DocumentTypeSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[document_types] WHERE " +
...
func (o *DocumentTypeSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT [dbo].[document_types].* FROM [dbo].[document_types] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, documentTypePrimaryKeyColumns, len(*o))
...
func DocumentTypeExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select case when exists(select top(1) 1 from [dbo].[document_types] where [id]=$1) then 1 else 0 end"
