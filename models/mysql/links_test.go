...
func testLinksExists(t *testing.T) {
	...
	e, err := LinkExists(ctx, tx, o.SourceID, o.DestinationID)
	if err != nil {
...
func testLinksFind(t *testing.T) {
	...
	linkFound, err := FindLink(ctx, tx, o.SourceID, o.DestinationID)
	if err != nil {
		t.Error(err)
	}
...
var (
	linkDBTypes = map[string]string{`SourceID`: `int`, `DestinationID`: `int`}
	_           = bytes.MinRead
)
