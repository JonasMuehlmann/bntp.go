...
func testDocumentToManyTags(t *testing.T) {
	...
	_, err = tx.Exec("insert into \"document_contexts\" (\"document_id\", \"tag_id\") values ($1, $2)", a.ID, b.ID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into \"document_contexts\" (\"document_id\", \"tag_id\") values ($1, $2)", a.ID, c.ID)
	if err != nil {
		t.Fatal(err)
	}
...
func testDocumentToManySourceDocuments(t *testing.T) {
	...
	_, err = tx.Exec("insert into \"links\" (\"destination_id\", \"source_id\") values ($1, $2)", a.ID, b.ID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into \"links\" (\"destination_id\", \"source_id\") values ($1, $2)", a.ID, c.ID)
	if err != nil {
		t.Fatal(err)
	}
...
func testDocumentToManyDestinationDocuments(t *testing.T) {
	...
	_, err = tx.Exec("insert into \"links\" (\"source_id\", \"destination_id\") values ($1, $2)", a.ID, b.ID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into \"links\" (\"source_id\", \"destination_id\") values ($1, $2)", a.ID, c.ID)
	if err != nil {
		t.Fatal(err)
	}
...
var (
	documentDBTypes = map[string]string{`ID`: `integer`, `Path`: `text`, `DocumentTypeID`: `integer`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`, `DeletedAt`: `timestamp without time zone`}
	_               = bytes.MinRead
)
